% Analysis_Associative_Generators_RD_MDD_L1
clear; close all; clc
...
for iterr = 1
    ...
    for k = 4
        ...
        for fn = 1 : size( flist, 1 )
            ...
            vars = struct;
            vars.N_inputs = [ size( act_G{ 1, 1 }, 1 ) ];% numbers of input units, (1) X (N), where N represent the number of modality.
            vars = HSM_defaultStr( vars );
            vars = HSM_initialize( vars );

            vars = HSM_defaultAct( vars );
            vars.learning = 'off';% 'on', 'off', or 'assign'.

            [ vars, act ] = HSM_process_GPU( vars, [ act_G{ 1, 1 } ] );

            acth = HSM_split( vars, act );
            act_HSM_G = acth{ 4, 1 };

            % -------------------------------------------------------------------------

            vars = struct;
            vars.N_inputs = [ size( rec_G{ 1, 1 }, 1 ) ];% numbers of input units, (1) X (N), where N represent the number of modality.
            vars = HSM_defaultStr( vars );
            vars = HSM_initialize( vars );
            ...
            [ vars, act ] = HSM_process_GPU( vars, [ rec_G{ 1, 1 } ] );

            acth = HSM_split( vars, act );
            act_HSM_recG = acth{ 4, 1 };
            ...
            for beh = 1 : 4
                for iter = 1 : 30
                    ...
                    acc = idx_out == idx_Te;
                    acc_HSM_G( beh, iter ) = mean( acc, 1, 'omitnan' );
                end; clear iter
            end; clear beh
            ...
            for beh = 1 : 4
                for iter = 1 : 30
                    ...
                    idx_out = predict( Mdl, data_Te );
                    acc = idx_out == idx_Te;
                    acc_HSM_recG( beh, iter ) = mean( acc, 1, 'omitnan' );
                end; clear iter
            end; clear beh
            clear beh iter idxRandom idxBehavior idxBehavior_Tr idxBehavior_Te idxRandom_Tr idxRandom_Te idx_Tr idx_Te data_Tr data_Te Mdl idx_out acc
            ...
            save( [ 'Results_ExpRD', fname( 8 : end - 4 ), '_k', num2str( k ), '_iterr', num2str( iterr ), '.mat' ], 'acc_HSM_G', 'acc_HSM_recG', 'mean_eval_G_F' )
