...
%     for run = 1 : numRun
%         nii_file       = dir(fullfile( sub_dir, [ dlist( sub, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ] ));
...
%     for run = 1 : numRun
%         nii_file       = dir( fullfile( sub_dir, [ 'r', dlist( sub, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ] ) );
%         nii_file       = nii_file.name;
%         volumeInfo     = spm_vol(fullfile(sub_dir,nii_file));
%         numVolume      = length(volumeInfo);
% 
...
% 
%     for run = 1 : numRun
%         anat_run_file  = ['run' num2str(run,'%d') anat_file.name];
...
% 
%         mean_nii_file  = dir(fullfile(sub_dir,['mean' dlist( sub, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii']));
...
%     for run = 1 : numRun
%         anat_file      = dir( [sub_dir, 'run', num2str(run,'%d'), dlist( sub, 1 ).name, '_T1w.nii' ] );
%         anat_name      = anat_file.name;
...
%     for run = 1 : numRun
%         anat_file      = dir( [sub_dir, 'y_run', num2str(run,'%d'), dlist( sub, 1 ).name, '_T1w.nii' ] );
%         anat_name      = anat_file.name;
%         anat_file      = fullfile(sub_dir,anat_name);
%         disp(anat_file);
% 
%         nii_file       = dir(fullfile(sub_dir, [ 'ar', dlist( sub, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ]));
...
%         disp(anat_file);
% 
%         nii_file       = dir( [sub_dir, 'c1run', num2str(run,'%d'), dlist( sub, 1 ).name, '_T1w.nii' ] );
%         nii_name       = nii_file.name;
%         nii_file       = fullfile(sub_dir,nii_name);
%         disp(nii_file);
% 
...
%         disp(anat_file);
% 
%         nii_file       = dir( [sub_dir, 'c2run', num2str(run,'%d'), dlist( sub, 1 ).name, '_T1w.nii' ] );
%         nii_name       = nii_file.name;
%         nii_file       = fullfile(sub_dir,nii_name);
%         disp(nii_file);
% 
...
%     for run = 1 : numRun
%         nii_file       = dir(fullfile(sub_dir, [ 'war', dlist( sub, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ]));
%         nii_name       = nii_file.name;
%         nii_file       = fullfile(sub_dir,nii_name);
% 
...
%     move_voxel = [];
%     for run = 1 : numRun
%         fname = dir( [ 'rp_', dlist( dn, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.txt' ] );
...
%     for run = 1 : numRun
% 
%         fname = dir( [ dname, '_task-*_run-', num2str(run,'%d'), '_events.tsv' ] );
%         [ ~, ~, event_raw ] = tsvread( fname( 1, 1 ).name );
%         events{ run, 1 } = event_raw;
%         % events{ run, 1 } = importdata( fname( 1, 1 ).name );
%         
%         % fname = dir( [ 'swar', dlist( dn, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ] );
%         fname = dir( [ 'war', dlist( dn, 1 ).name, '_task-*_run-', num2str(run,'%d'), '_bold.nii' ] );
%         bold{ run, 1 } = niftiread( fname( 1, 1 ).name );
% 
%         dim_data = size( bold{ run, 1 }, 1 : 3 );
%         fname = dir( [ 'wc1run', num2str( run ), dlist( dn, 1 ).name, '_T1w.nii' ] );
...
%         c1{ run, 1 } = c_data;
%         fname = dir( [ 'wc2run', num2str( run ), dlist( dn, 1 ).name, '_T1w.nii' ] );
%         c_data = niftiread( fname( 1, 1 ).name );
%         % c_data( c_data < 255 / 2 ) = 0;
%         c_data( c_data < 255 / 128 ) = 0;
%         c_data( c_data > 0 ) = 1;
